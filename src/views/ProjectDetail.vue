...
const fetchRelatedProjects = async () => {
  ...
  const filteredProjects = projectsStore.projects.filter(p => p.id !== projectId)
...
onMounted(async () => {
  ...
  if (!projectId) {
    ...
    await projectsStore.fetchProjects()
    project.value = projectsStore.projects.find(p => p.id === projectId)
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <div class="flex gap-3" data-aos="fade-up" data-aos-delay="200">
            ...
            <Button 
              ...
              @click="window.open(project.sourceUrl, '_blank')"
            />
          </div>
        </div>
      </div>
